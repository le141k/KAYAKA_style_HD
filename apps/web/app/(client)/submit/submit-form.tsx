...
export function SubmitTicketForm() {
  ...
  const onSubmit = async (data: SubmitForm) => {
    try {
      ...
      // Handle error via toast in real impl
    }
  };
    ...
    return (
      ...
      >
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          ...
          <code className="font-mono font-bold text-foreground">{successMask}</code>,
          специалист скоро ответит.
        </p>
        <Button
          className="mt-6"
          variant="outline"
          onClick={() => setSuccessMask(null)}
        >
          Создать ещё одно
        </Button>
      </motion.div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          <Select
            defaultValue="normal"
            onValueChange={(v) =>
              setValue("priority", v as SubmitForm["priority"])
            }
          >
        ...
        <div className="space-y-1.5">
          ...
          <Select onValueChange={(v) => setValue("department_id", v)}>
            ...
            <SelectContent>
              <SelectItem value="1">Техническая поддержка</SelectItem>
              <SelectItem value="2">Биллинг</SelectItem>
              <SelectItem value="3">NOC</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
