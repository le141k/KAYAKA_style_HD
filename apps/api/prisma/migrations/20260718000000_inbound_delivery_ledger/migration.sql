...
-- Safe upgrade / cutover: any already-enabled IMAP queue is HALTED
-- (NEEDS_RECONCILIATION) so the ledger poller cannot FROM_NOW-bootstrap over an
-- in-flight legacy cursor and silently skip mail delivered during the deploy. The
-- actual cursor migration is a deliberate, audited operator action — the reconcile
-- endpoint (POST /api/admin/email-queues/:id/reconcile) reads the legacy Setting
-- state (`imap/state:<id>` primary, `imap/lastSeenUid:<id>` fallback), carries the
-- UIDVALIDITY + watermark forward, and only then clears the halt. We intentionally
-- do NOT copy the cursor here: leaving lastSeenUid = 0 while halted is inert (a
-- halted queue never polls), and keeping a single source of cutover truth avoids two
-- divergent cursor-migration paths.
UPDATE "EmailQueue" q
SET "syncState" = 'NEEDS_RECONCILIATION',
    "lastError" = 'Upgraded to InboundDelivery ledger; run reconcile (RESUME_MIGRATED / FROM_NOW / bounded BACKFILL) to resume polling'
