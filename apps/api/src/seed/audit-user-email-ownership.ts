...
export async function auditUserEmailOwnership(client: PrismaClient = prisma): Promise<OwnershipAudit> {
  ...
  // Exact group totals (not limited by SAMPLE_LIMIT). Aggregate queries always return one row.
  const groupTotals = await client.$queryRaw<{ groups: bigint; ambiguous: bigint }[]>`
  ...
  `;
  const dupGroupTotal = groupTotals[0]?.groups;
  const ambiguousGroupTotal = groupTotals[0]?.ambiguous;

  // 2. UserEmail rows still not normalized (collided during the migration).
  const unnormRows = await client.$queryRaw<{ c: bigint }[]>`
    SELECT count(*) AS c FROM "UserEmail" WHERE "email" <> lower(btrim("email", ${WS}))
  `;
  const unnormalizedRowCount = unnormRows[0]?.c;
