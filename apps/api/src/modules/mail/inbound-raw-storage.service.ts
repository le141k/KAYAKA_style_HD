...
export class InboundRawStorageService {
  ...
  async write(bytes: Buffer): Promise<string> {
    ...
    const id = randomUUID();
    const storageKey = `${InboundRawStorageService.PREFIX}/${id}.eml`;
    ...
    try {
      ...
      this.logger.error(`Inbound raw MIME write failed: ${String(err)}`);
  ...
  async commit(storageKey: string): Promise<void> {
    try {
      ...
      if (err instanceof ServiceUnavailableException) throw err;
      this.logger.error(`Inbound raw MIME marker commit failed: ${String(err)}`);
  ...
  async read(storageKey: string): Promise<Buffer> {
    try {
      ...
      if (err instanceof ServiceUnavailableException) throw err;
      this.logger.error(`Inbound raw MIME read failed: ${String(err)}`);
  ...
  async remove(storageKey: string): Promise<void> {
    try {
      ...
      if (err instanceof ServiceUnavailableException) throw err;
      this.logger.error(`Inbound raw MIME cleanup failed: ${String(err)}`);
  ...
  async listPending(limit: number, olderThan: Date): Promise<string[]> {
    ...
    try {
      ...
      if (err instanceof ServiceUnavailableException) throw err;
      this.logger.error(`Inbound raw MIME pending-marker scan failed: ${String(err)}`);
  ...
  async capacity(): Promise<{ availableBytes: bigint; reserveBytes: bigint }> {
    try {
      ...
      if (err instanceof ServiceUnavailableException) throw err;
      this.logger.error(`Inbound raw MIME capacity probe failed: ${String(err)}`);
