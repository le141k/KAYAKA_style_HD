...
export class JwtAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Main's per-staff Redis access cutoff (best-effort, fail-open): fast-reject an access
    // token minted before a revokeAllForStaff cutoff, even before the DB check below.
    if (
      this.blocklist &&
