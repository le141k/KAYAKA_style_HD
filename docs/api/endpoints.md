...
> Optional `x-inbound-queue-id` header (positive integer, else **400**) — binds the message to a
...
| Method | Path                                                           | Auth            | Body                                                                                         | Returns                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------ | -------------------------------------------------------------- | --------------- | -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | /api/admin/email-queues                                        | 🔒 `admin.mail` | —                                                                                            | `EmailQueue[]` (no password; incl. `syncState`, `lastError`, `lastSeenUid`, `uidValidity`, `cursorGeneration`, `bootstrapPolicy`, `bootstrapBackfillLimit`, and the liveness stamps `lastConnectedAt` / `lastPollAt` / `lastAcceptedAt`)                                                                                                                                                                                                                                                                                                                                                                       |
...
| PUT    | /api/admin/email-queues/{id}                                   | 🔒 `admin.mail` | partial                                                                                      | Updated queue. **Mailbox-identity guard:** on a bootstrapped IMAP queue, changing `host` / `port` / `username` / `useTls` resets and HALTS the cursor (`syncState=NEEDS_RECONCILIATION`, `uidValidity`/`lastSeenUid` cleared, `cursorGeneration` bumped) so the stale UID cursor can't resume against a different mailbox; a **password-only change is exempt**                                                                                                                                                                                                                                                |
| DELETE | /api/admin/email-queues/{id}                                   | 🔒 `admin.mail` | —                                                                                            | 204 No Content                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| POST   | /api/admin/email-queues/{id}/reconcile                         | 🔒 `admin.mail` | `{ mode: 'RESUME_MIGRATED' \| 'FROM_NOW' \| 'BACKFILL', reason?, confirm?, backfillLimit? }` | Cutover off the legacy Setting cursor. **Refuses a non-IMAP queue → 400.** `RESUME_MIGRATED` carries `imap/state:<id>` (UIDVALIDITY+watermark, rewound past pending UIDs) forward and lands `OK`; `FROM_NOW` (requires `confirm:true`+`reason`) discards it; `BACKFILL` (requires `backfillLimit ≥ 1`) re-bootstraps. **FROM_NOW/BACKFILL leave the queue `BOOTSTRAPPING`** until the poller fixes the baseline. Bumps `cursorGeneration`; writes a durable `InboundAuditLog` row (actor + reason + before/after cursor) in addition to the structured log line. Returns `{ reconciled, mode, queue, detail }` |
| GET    | /api/admin/email-queues/inbound/health                         | 🔒 `admin.mail` | —                                                                                            | `{ queues[], ledger: { backlog, byState, stalledProcessing, oldestPendingAt, lastProcessedAt }, alerts[], checkedAt }` — halt / quarantine / stalled-lease / aged-backlog signals for a dashboard or probe. Each `queues[]` entry includes `syncState`, `lastError`, cursor fields, and the liveness stamps `lastConnectedAt` / `lastPollAt` / `lastAcceptedAt`                                                                                                                                                                                                                                                |
| GET    | /api/admin/email-queues/inbound/quarantine                     | 🔒 `admin.mail` | —                                                                                            | Quarantined `InboundDelivery[]` (metadata only)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| POST   | /api/admin/email-queues/inbound/quarantine/{deliveryId}/replay | 🔒 `admin.mail` | —                                                                                            | `{ replayed: true }` — resets the delivery to ACCEPTED (attempts 0, lease cleared) so the drain reprocesses it (404 if not quarantined); writes a durable `InboundAuditLog` row (actor)                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Method | Path                                  | Auth            | Body | Returns                                                                                                                                                               |
| ------ | ------------------------------------- | --------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| POST   | /api/admin/outbound-emails/{id}/retry | 🔒 `admin.mail` | —    | Requeues only `FAILED`, `AMBIGUOUS` or `RETRY` durable mail using its original Message-ID, recipient and attachment snapshots. `SENT` is never retried by this route. |
