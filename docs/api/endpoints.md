...
> the controller (defence in depth). With a valid secret and normal inbound delivery disabled,
> the pre-parser middleware returns a retryable **503** before reading raw MIME. Active IMAP
> capture-only mode also returns **503 for every PIPE request before the body parser**; it never
> turns a selected test id into an MTA raw-MIME sink. The body may be raw
> `message/rfc822` / `application/octet-stream` bytes **or** JSON
> `{ raw }`. Outside capture-only, accepted mail feeds the normal
...
> X-Loop / self-from). Both `x-inbound-delivery-id` and `x-inbound-queue-id` are **required** and
> malformed/missing values receive **400 before MIME buffering**. Delivery id is 1–256 safe UTF-8
> bytes and is SHA-256 hashed for the transport key. Queue must
...
| POST / PUT / DELETE | /api/admin/email-queues[/{id}]                                 | 🔒 `mail.view` + `mail.configure`       | Queue fields, including IMAP `mailbox` folder                        | Create/update/delete. An IMAP queue selects its explicit mailbox/folder rather than implicitly sharing `INBOX`; identity/folder changes atomically advance mailbox epoch, halt for reconciliation and invalidate old cursor snapshots; password-only change does not. A `captureRetiredAt` queue cannot be re-enabled or deleted and is never eligible for fresh normal ingress. |
| POST                | /api/admin/email-queues/{id}/reconcile                         | 🔒 `mail.view` + `mail.reconcile`       | `{mode, expectedCursorGeneration, reason, confirm?, backfillLimit?}` | Server returns `allowedModes`; standard reconcile rejects a healthy queue, enforces cause/mode, generation/epoch/state CAS, exact IMAP baseline and transactional requested+completed/failed audit. `FROM_NOW` requires confirmation/reason; BACKFILL requires bounded positive limit. A capture-retired queue cannot reconcile into fresh ingress.                              |
| GET                 | /api/admin/email-queues/inbound/health                         | 🔒 `mail.view`                          | —                                                                    | Per-queue liveness plus ledger backlog/stalled work/quarantine and capture-only counts/bytes, raw-storage reserve and actionable alerts. `captureTarget.ready` is true only after this API process live-verifies the selected dedicated empty IMAP folder; a retired queue remains visibly marked as non-reusable evidence.                                                      |
...
| POST                | /api/admin/email-queues/inbound/quarantine/{deliveryId}/replay | 🔒 `mail.view` + `mail.replay`          | `{reason, expectedUpdatedAt}`                                        | Transactional CAS replay + durable audit. Stale/repeated action → 409; truncated MIME → 400/replay blocked; capture-only mode → 409.                                                                                                                                                                                                                                             |
| GET                 | /api/admin/email-queues/inbound/captured                       | 🔒 `mail.view`                          | `?page&limit&queueId&reason&messageId&from&to`                       | Department-scoped `CAPTURED` metadata only; raw MIME and opaque storage keys are never returned.                                                                                                                                                                                                                                                                                 |
| GET                 | /api/admin/email-queues/inbound/captured/{deliveryId}          | 🔒 `mail.view`                          | —                                                                    | Department-scoped captured metadata plus safe audit timeline; no raw MIME/storage key.                                                                                                                                                                                                                                                                                           |
| POST                | /api/admin/email-queues/inbound/captured/{deliveryId}/promote  | 🔒 `mail.view` + `mail.capture.promote` | `{reason, expectedUpdatedAt}`                                        | Audited, transactional `CAPTURED → ACCEPTED` CAS. Promotion is refused while capture-only mode is active and is constrained to the configured capture-retired normal-canary queue/delivery; it is a deliberate transition into normal processing, not a preview or a way to reopen fresh ingress.                                                                                |
...
| POST                | /api/admin/workflow-email-events/{eventId}/replay              | 🔒 `mail.view` + `mail.replay`          | `{reason, expectedUpdatedAt}`                                        | Requeues only safe quarantined events under a ticket-scope/version CAS and writes `TicketAuditLog`. Invalid snapshots or changed requester identity remain blocked.                                                                                                                                                                                                              |
...
| Method | Path                                  | Auth                              | Body | Returns                                                                                                                                                               |
| ------ | ------------------------------------- | --------------------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| POST   | /api/admin/outbound-emails/{id}/retry | 🔒 `mail.view` + `mail.configure` | —    | Requeues only `FAILED`, `AMBIGUOUS` or `RETRY` durable mail using its original Message-ID, recipient and attachment snapshots. `SENT` is never retried by this route. |
