...
| Model                       | Key columns                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Relations                                                                                                                                                |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **EmailQueue**              | `id`, `type EmailQueueType`, `emailAddress`, `host`, `port Int` (default 993), `username`, `passwordEnc` (encrypted at rest), `useTls Bool`, `departmentId?`, `signature`, `routingPriority Int` (default 100; lower wins, `id` breaks ties), `sendAutoresponder Bool`, `isEnabled Bool`, `createdAt`; **IMAP cursor** `lastSeenUid BigInt`, `uidValidity BigInt?` (NULL = never bootstrapped), `syncState EmailQueueSyncState`, `lastError?`, `cursorGeneration Int`; **reconcile intent** `bootstrapPolicy EmailQueueBootstrapPolicy?` + `bootstrapBackfillLimit Int?` (per-queue override of the global bootstrap policy, consumed at next bootstrap); **liveness** `lastConnectedAt?`, `lastPollAt?`, `lastAcceptedAt?` (advisory operator stamps — supervisor connect / poller cycle / durable accept; not part of any CAS) | Optional FK to `Department`; receiving/routed relations to inbound ledger/claims                                                                         |
...
| **InboundDelivery**         | `id`, `transport InboundTransport`, receiving `queueId?`, `transportKey` **@unique** (transport retry claim), `uidValidity BigInt?`, `uid?`, `externalId?`, legacy compatibility `messageId?` **@unique** (retained through the quiesced expand/contract cutover), non-unique `observedMessageId?`, `messageIdHash?`, `semanticHash?`, `contentHash`, `envelopeFrom?/To?`, `subject`, receiving `departmentId?`, deterministic `routedQueueId?` / `routedDepartmentId?`, `rawMime Bytes?` (durable, replayable), `rawStorageKey?`, `rawPrunedAt?`, `truncated Bool`, `sizeBytes`, lifecycle/lease/ticket fields; indexes `(state,nextAttemptAt)`, `(queueId,uidValidity,uid)`, `(messageIdHash)`, `(observedMessageId)`                                                                                                          | FKs to receiving/routed `EmailQueue` / `Department`, optional logical `InboundMessageClaim`                                                              |
| **InboundMessageClaim**     | `messageIdHash CHAR(64)` **PK** (SHA-256 normalized real RFC Message-ID), diagnostic `normalizedMessageId`, `semanticHash?` + version (`NULL`/0 is fail-closed historical backfill), unique `winnerDeliveryId`, immutable `routedQueueId?` / `departmentId?`, `ticketId?`, `postId?`, timestamps                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Winner is one `InboundDelivery`; all observed transport copies relate through `messageIdHash`; route/ticket/post FKs retain forensic linkage             |
| **OutboundEmail**           | opaque CUID, `ticketId`, unique `postId`, optional `emailQueueId`, unique pre-generated `messageId`, immutable from/reply-to/subject/html/text/threading snapshots, state `QUEUED/PROCESSING/SENT/RETRY/FAILED/AMBIGUOUS`, attempts/retry time, lease owner+version, sanitized diagnostics and SMTP accept timestamps                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | One public staff `TicketPost`; has immutable recipient and attachment snapshots; DB is authoritative queue, BullMQ carries only its id                   |
| **OutboundEmailRecipient**  | `outboundEmailId`, normalized email, role `TO/CC/BCC`; unique `(outboundEmailId,email)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Snapshot owned by `OutboundEmail`; BCC is never in staff timeline/API projection                                                                         |
| **OutboundEmailAttachment** | `outboundEmailId`, unique source attachment, filename/MIME/size/SHA-1/storage key snapshot                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Snapshot owned by `OutboundEmail`; direct service delete of the source is blocked while referenced, while ticket/post cascade removes the coherent graph |
| **InboundAuditLog**         | `id`, `actorStaffId?` (plain int, no FK — survives actor removal; null for scheduled/system actions), `actorEmail`, `action` (`mail.reconcile` \| `mail.quarantine_replay`), `queueId?`, `deliveryId?`, `reason?`, `metadata Json`, `createdAt`; indexes `(createdAt)`, `(queueId)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | append-only; kept separate from `RbacAuditLog` so mail-ops history is queryable per queue/delivery                                                       |
...
| `EmailQueueSyncState`       | `OK`, `BOOTSTRAPPING`, `NEEDS_RECONCILIATION`                                                          | `EmailQueue.syncState` (`BOOTSTRAPPING` is transient: a FROM_NOW/BACKFILL reconcile parks the queue here until the poller fixes its high-water baseline and the generation-guarded CAS flips it to `OK` — so health never reports OK before a queue has a cursor) |
...
| `InboundDelivery`     | `(state, nextAttemptAt)`, `(queueId, uidValidity, uid)`, `messageIdHash`, `observedMessageId`, `routedQueueId` |
