...
> _Last regenerated: 2026-07-22._
...
| **TicketPost**     | `id`, `ticketId`, `authorType ActorType`, `staffId?`, `userId?`, `fullName`, `email`, `subject`, `contents` (HTML or plaintext), `isHtml Bool`, `isEmailed Bool`, `isThirdParty Bool`, `creationMode CreationMode`, non-unique `messageId?` (RFC threading), unique nullable `inboundMessageId?` (inbound idempotency only), `ipAddress`, `editedAt?`, `editedByStaffId?`, `createdAt`                                                                                                                                                                                                                                                     | Belongs to `Ticket` (cascade delete); optional `Staff`/`User`; has many `Attachment`; public staff post has at most one durable `OutboundEmail`                                                                                                                                                                                                                            |
| **TicketNote**     | `id`, `ticketId`, `staffId?`, `contents`, `createdAt`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Belongs to `Ticket` (cascade delete); optional `Staff` — internal notes only                                                                                                                                                                                                                                                                                               |
| **Attachment**     | `id`, `ticketId?`, `postId?`, `fileName`, `mimeType`, `size Int`, `sha1`, `storageKey` (path/key in object store), `createdAt`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Optional FK to `Ticket` and `TicketPost` (both cascade delete); direct service delete is blocked while an `OutboundEmailAttachment` snapshot references it, while DB cascade supports coherent ticket/outbox removal                                                                                                                                                       |
...
| Model                       | Key columns                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Relations                                                                                          |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------- |
| **EmailQueue**              | Transport config plus `departmentId?`, `isEnabled`, `configGeneration`; **IMAP snapshot** `lastSeenUid BigInt`, `uidValidity BigInt?`, `mailboxEpoch Int`, `cursorGeneration Int`, `syncState`, `reconcileCause?`, `reconcileRequestedAt?`; bootstrap intent and `routingPriority Int` (lower wins, queue-id tie-break); safe liveness stamps `lastConnectionAttemptAt?`, `lastConnectedAt?`, `lastDisconnectedAt?`, `lastConnectionErrorAt?`, `lastPollStartedAt?`, `lastPollCompletedAt?`, `lastAcceptedAt?`. Credentials are never selected into admin responses.     | Optional FK to `Department`; has many `InboundDelivery`                                            |
...
| **InboundDelivery**         | One received transport copy: unique `transportKey`; `queueId?`, `mailboxEpoch?`, `uidValidity?`, `uid?`, `externalId?`; non-unique `observedMessageId?`, `messageIdHash?`, `semanticHash?`, raw `contentHash`; receiving and persisted routed queue/department/autoresponder-policy snapshots; inline `rawMime?` or opaque `rawStorageKey?`, `rawPrunedAt?`, `truncated`; lifecycle/lease/ticket/post timestamps. `rawStorageKey` is private and never returned by quarantine API. Indexes include `(state,nextAttemptAt)` and `(queueId,mailboxEpoch,uidValidity,uid)`. | FK to `EmailQueue`; optional logical claim                                                         |
| **InboundMessageClaim**     | Atomic logical-message record keyed by `messageIdHash`; normalized real Message-ID, semantic hash, winner delivery, deterministic routed queue/department/autoresponder policy, ticket/post and timestamps.                                                                                                                                                                                                                                                                                                                                                              | One winner delivery; duplicate/conflict deliveries remain in `InboundDelivery`                     |
| **InboundRawMimeStaging**   | `storageKey` PK, lease expiry and creation time. A large raw file has this row until acceptance consumes it transactionally; the reaper locks expired rows before removing files.                                                                                                                                                                                                                                                                                                                                                                                        | No customer-facing relation; fences filesystem staging.                                            |
| **OutboundEmail**           | Opaque CUID, optional `ticketId`/unique `postId`/unique `reportRunId`, optional queue/SLA event, unique `idempotencyKey?`, kind `STAFF_REPLY/AUTORESPONDER/AUTO_CLOSE/WORKFLOW/REPORT/INTERNAL_NOTIFICATION`, unique pre-generated Message-ID, immutable from/reply/subject/body/threading snapshot; state `QUEUED/PROCESSING/SENT/RETRY/FAILED/AMBIGUOUS`, attempts, backoff, lease owner/version and sanitized diagnostics.                                                                                                                                            | Staff post, scheduled report, SLA event or ticket; PostgreSQL is the authoritative queue.          |
| **OutboundEmailRecipient**  | Outbox id, normalized email, `TO/CC/BCC`; unique `(outboundEmailId,email)`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Immutable recipient snapshot; BCC is never exposed in staff ticket projections.                    |
| **OutboundEmailAttachment** | Outbox id, source attachment id, filename/MIME/size/hash/storage-key snapshot; unique source per outbox.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Direct source deletion is rejected while retained; ticket/post cascade removes the coherent graph. |
| **InboundAuditLog**         | Append-only actor/action/reason/metadata history for reconcile requested/completed/failed, quarantine replay and transport/semantic conflicts.                                                                                                                                                                                                                                                                                                                                                                                                                           | Separate from `RbacAuditLog`; queryable by queue/delivery                                          |
| **WorkflowEmailEvent**      | Immutable customer-workflow action snapshot, unique business `sourceKey`, state/attempt/lease/error timestamps. Invalid legacy action snapshots remain `QUARANTINED`; operator replay is CAS-fenced and audited.                                                                                                                                                                                                                                                                                                                                                         | Ticket-owned; materializes idempotent `WORKFLOW` outbox commands.                                  |
| **SlaEscalationEvent**      | Unique SLA breach source (`ticketId`, breach target and `sourceKey`) created with escalation effects.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Ticket-owned; may own several internal-notification commands.                                      |
...
| `EmailQueueSyncState`       | `OK`, `BOOTSTRAPPING`, `NEEDS_RECONCILIATION`                                                                           | `BOOTSTRAPPING` exists only while the reconcile request captures and durably writes an exact IMAP baseline; success is returned only after it becomes `OK` |
| `EmailQueueReconcileCause`  | `LEGACY_MIGRATION`, `UIDVALIDITY_CHANGED`, `MAILBOX_IDENTITY_CHANGED`, `MANUAL_FORCE`, `TRANSPORT_COLLISION`, `UNKNOWN` | Server-side reconcile policy / fail-closed operator diagnostics                                                                                            |
| `EmailQueueBootstrapPolicy` | `FROM_NOW`, `BACKFILL`                                                                                                  | `EmailQueue.bootstrapPolicy` (per-queue reconcile intent)                                                                                                  |
...
| `InboundDelivery`         | `(state, nextAttemptAt)`, `(queueId, mailboxEpoch, uidValidity, uid)`, `messageIdHash`, `observedMessageId`, `routedQueueId` |
| `InboundMessageClaim`     | `messageIdHash` (PK), `winnerDeliveryId` (unique), `routedQueueId`, `departmentId`, `ticketId`                               |
| `InboundAuditLog`         | `createdAt`, `queueId`                                                                                                       |
| `InboundRawMimeStaging`   | `leaseExpiresAt`                                                                                                             |
| `OutboundEmail`           | `ticketId`, `(state,nextAttemptAt)`, `leaseExpiresAt`                                                                        |
| `OutboundEmailRecipient`  | `(outboundEmailId, role)`                                                                                                    |
| `OutboundEmailAttachment` | `sourceAttachmentId`                                                                                                         |
...
| `20260717100000_ticket_post_message_id_unique`           | ticket_post_message_id_unique           | Historical migration that made `TicketPost.messageId` unique after cleaning legacy empty values. Its overloaded inbound role is superseded by `20260723040000_ticket_post_inbound_message_id`, which keeps `messageId` non-unique for threading and moves inbound idempotency to its own key.                                                                                                                                                                                                                                                                             |
| `20260718000000_inbound_delivery_ledger`                 | inbound_delivery_ledger                 | Adds the `InboundDelivery` ledger table (unique `transportKey`, unique `messageId`), the `InboundTransport` / `InboundDeliveryState` / `EmailQueueSyncState` (`OK`,`NEEDS_RECONCILIATION`) enums, and the `EmailQueue` durable IMAP cursor columns (`lastSeenUid`, `uidValidity`, `syncState`, `lastError`, `cursorGeneration`). **Halts every already-ENABLED IMAP queue** (`NEEDS_RECONCILIATION`) so cutover is an explicit, audited operator reconcile rather than a FROM_NOW over an in-flight legacy cursor (ADR-0007)                                              |
| `20260718010000_email_queue_bootstrap_policy`            | email_queue_bootstrap_policy            | Adds the `EmailQueueBootstrapPolicy` enum + `EmailQueue.bootstrapPolicy` / `bootstrapBackfillLimit` — per-queue reconcile intent that overrides the global `TELECOM_HD_IMAP_BOOTSTRAP_POLICY` for one queue's next bootstrap                                                                                                                                                                                                                                                                                                                                              |
| `20260721000000_email_queue_bootstrapping_state`         | email_queue_bootstrapping_state         | Adds the transient `BOOTSTRAPPING` value to `EmailQueueSyncState`. Isolated in its own migration because Postgres forbids `USING` a newly added enum value in the same transaction that adds it; nothing here references the value                                                                                                                                                                                                                                                                                                                                        |
| `20260721010000_inbound_hardening`                       | inbound_hardening                       | Adds `EmailQueue.lastConnectedAt` / `lastPollAt` / `lastAcceptedAt` liveness stamps, `InboundDelivery.truncated` + `rawPrunedAt`, and the `InboundAuditLog` table (+ `createdAt` / `queueId` indexes). **Also halts DISABLED legacy IMAP queues** (`NEEDS_RECONCILIATION`) — the ledger migration halted only enabled ones, so a disabled queue that was later re-enabled could FROM_NOW over its legacy cursor and skip mail                                                                                                                                             |
| `20260722020000_mailbox_epoch_reconcile_fence`           | mailbox_epoch_reconcile_fence           | Adds mailbox identity epoch, typed reconcile cause/request time and an epoch-aware IMAP delivery index/key migration. Legacy halted queues are classified fail-closed; only the known ledger-upgrade marker permits `RESUME_MIGRATED`.                                                                                                                                                                                                                                                                                                                                    |
| `20260723010000_inbound_message_claims`                  | inbound_message_claims                  | Expand-only P1-E: adds `EmailQueue.routingPriority`, non-unique observed/semantic/routed fields on `InboundDelivery`, and `InboundMessageClaim`. Uses approved PostgreSQL `pgcrypto` solely to SHA-256 backfill legacy non-null delivery IDs with semantic version 0 (future matching copies fail closed); keeps the legacy unique `InboundDelivery.messageId` until a separately verified post-quiesce cutover migration.                                                                                                                                                |
| `20260722140000_inbound_operator_liveness`               | inbound_operator_liveness               | Adds attempt/disconnect/error and poll-start/poll-complete timestamps. These are advisory operator telemetry, never cursor correctness state.                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `20260722141000_split_mail_permissions`                  | split_mail_permissions                  | Forward-only backfill from historical `admin.mail` to `mail.view`, `mail.replay`, `mail.reconcile`, `mail.configure`; preserves custom legacy grants, gives Manager read-only visibility, removes the old key.                                                                                                                                                                                                                                                                                                                                                            |
| `20260723020000_durable_outbound_outbox`                 | durable_outbound_outbox                 | Adds `OutboundEmail`, recipient and attachment snapshot tables plus delivery/recipient enums. No existing mail is backfilled; only new staff replies use the transactional outbox.                                                                                                                                                                                                                                                                                                                                                                                        |
| `20260723030000_inbound_raw_mime_staging_fence`          | inbound_raw_mime_staging_fence          | Adds a short-lived DB staging lease for filesystem raw-MIME writes. Acceptance consumes it transactionally and the reaper locks expired rows before deleting a file.                                                                                                                                                                                                                                                                                                                                                                                                      |
| `20260723040000_ticket_post_inbound_message_id`          | ticket_post_inbound_message_id          | Separates inbound post idempotency (`TicketPost.inboundMessageId @unique`) from non-unique RFC threading (`TicketPost.messageId`), backfills existing EMAIL posts, and retains a non-unique threading lookup index. `OutboundEmail.messageId` remains unique.                                                                                                                                                                                                                                                                                                             |
| `20260723050000_inbound_acceptance_and_automated_outbox` | inbound_acceptance_and_automated_outbox | Adds `EmailQueue.configGeneration`, immutable inbound autoresponder/routing snapshots, report/schedule generation fences, `WorkflowEmailEvent`, `SlaEscalationEvent`, and `OutboundEmailKind` (`AUTORESPONDER`, `AUTO_CLOSE`, `WORKFLOW`, `REPORT`, `INTERNAL_NOTIFICATION`). Makes ticket/post nullable where the durable command has another source, adds report/idempotency/SLA links, and retains the staff-reply post unique index. Idempotently provisions five required English customer/staff notification templates without overwriting operator customizations. |
...
- Per-queue IMAP UID watermarks originally lived in the `Setting` table (section `imap`).
  **Superseded (2026-07) by the `InboundDelivery` ledger + the `EmailQueue` durable cursor columns**
  (`lastSeenUid`/`uidValidity`/`syncState`/`cursorGeneration`); the legacy `imap/state:<id>` /
  `imap/lastSeenUid:<id>` settings are now read only by the `RESUME_MIGRATED` reconcile path during
  cutover (ADR-0007).
