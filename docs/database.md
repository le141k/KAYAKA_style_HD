...
| **InboundDelivery**         | One received transport copy: unique `transportKey`; `queueId?`, `mailboxEpoch?`, `uidValidity?`, `uid?`, `externalId?`; non-unique `observedMessageId?`, `messageIdHash?`, `semanticHash?`, raw `contentHash`; receiving and persisted routed queue/department snapshots; inline `rawMime?` or opaque `rawStorageKey?`, `rawPrunedAt?`, `truncated`; lifecycle/lease/ticket/post timestamps. `rawStorageKey` is private and never returned by quarantine API. Indexes include `(state,nextAttemptAt)` and `(queueId,mailboxEpoch,uidValidity,uid)`. | FK to `EmailQueue`; optional logical claim                                                         |
| **InboundMessageClaim**     | Atomic logical-message record keyed by `messageIdHash`; normalized real Message-ID, semantic hash, winner delivery, deterministic routed queue/department, ticket/post and timestamps.                                                                                                                                                                                                                                                                                                                                                              | One winner delivery; duplicate/conflict deliveries remain in `InboundDelivery`                     |
...
| **OutboundEmail**           | Opaque CUID, `ticketId`, unique `postId`, optional queue, unique pre-generated Message-ID, immutable from/reply/subject/body/threading snapshot; state `QUEUED/PROCESSING/SENT/RETRY/FAILED/AMBIGUOUS`, attempts, backoff, lease owner/version and sanitized diagnostics.                                                                                                                                                                                                                                                                           | One public staff `TicketPost`; PostgreSQL is the authoritative queue.                              |
