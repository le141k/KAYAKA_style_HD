...
| **RefreshToken** | `id` (UUID), `staffId`, `jti` (unique — opaque rotation id), `familyId` (indexed — rotation chain), `authVersion Int` (issue-time staff authVersion — stale rows are rejected on refresh), `tokenHash` (unique), `expiresAt`, `revokedAt?`, `createdAt` | belongs to `Staff` (cascade delete)                                                                                                                                           |
...
| `20260716180000_normalize_user_email_ownership` | normalize_user_email_ownership | Data-only, idempotent: normalizes existing `UserEmail.email` (trim+lowercase) for rows that don't collide, and backfills `Ticket.userId` from the normalized requester email **only when it maps to exactly one user**. Ambiguous/orphan rows are left untouched for the audit report. Does **not** add the case-insensitive `UNIQUE(email)` invariant yet (GOAL_PUBLIC_SECURITY S2-2) |
