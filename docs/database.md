...
| Model                   | Key columns                                                                                                                                                                                                                           | Relations                                                                        |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| **EmailQueue**          | connection fields; `routingPriority`; IMAP cursor `lastSeenUid BigInt`, `uidValidity?`, `cursorGeneration`, `mailboxEpoch`; typed state `syncState`, `reconcileCause?`, `reconcileRequestedAt?`; bootstrap intent and liveness stamps | Optional FK to `Department`; receiving/routed relations to inbound ledger/claims |
| **InboundDelivery**     | unique `transportKey`; IMAP `mailboxEpoch?` + `uidValidity?` + `uid?`; compatibility `messageId?` **@unique**; non-unique observed/semantic/route fields; raw/retry/lease/ticket fields                                               | FKs to receiving/routed queues/departments and optional `InboundMessageClaim`    |
| **InboundMessageClaim** | SHA-256 `messageIdHash` PK; normalized ID, semantic version/hash, unique winner delivery, immutable route/ticket/post snapshots                                                                                                       | Winner delivery plus all observed transport copies                               |
| **InboundAuditLog**     | append-only operator/system action, queue/delivery, reason and JSON metadata                                                                                                                                                          | Kept separate from `RbacAuditLog`                                                |
