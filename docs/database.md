...
| Model               | Key columns                                                                                                                                                                                                                                                                                                                                                             | Relations                                                                                          |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| **EmailQueue**      | `id`, `type EmailQueueType`, `emailAddress`, connection fields, `departmentId?`, `isEnabled`; **IMAP cursor** `lastSeenUid BigInt`, `uidValidity BigInt?`, `syncState`, `cursorGeneration`, durable `mailboxEpoch Int` (identity generation), safe `lastError?`; typed reconcile state `reconcileCause?`, `reconcileRequestedAt?`; bootstrap intent and liveness stamps | Optional FK to `Department`; has many `InboundDelivery`                                            |
...
| **InboundDelivery** | `id`, `transport InboundTransport`, `queueId?`, `transportKey` **@unique**, IMAP `mailboxEpoch?` + `uidValidity?` + `uid?`, `externalId?`, `messageId?` **@unique**, `contentHash`, envelope metadata, acceptance `departmentId?`, raw/retry/lease/ticket fields; indexes `(state,nextAttemptAt)`, `(queueId,mailboxEpoch,uidValidity,uid)`                             | FK to `EmailQueue`                                                                                 |
...
| `InboundDelivery`    | `(state, nextAttemptAt)`, `(queueId, mailboxEpoch, uidValidity, uid)`               |
