...
| `MailModule`           | `src/modules/mail/`           | Outbound SMTP/templates; inbound IMAP + authenticated PIPE acceptance into the durable ledger; deterministic ticket threading/routing, operator health/quarantine/reconcile, and BullMQ `mail` queue + `MailProcessor` |
