...
| `MailModule`           | `src/modules/mail/`           | Inbound IMAP/PIPE ledger and SMTP. Public staff reply creates a PostgreSQL `OutboundEmail` command atomically with its post; BullMQ holds only its id and recovery scans DB rows after Redis/worker restart. Legacy system templates remain compatible but are not yet durable outbox commands. |
...
| -------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `InboundMailService` | `setInterval` (60 s poll + 30 s drain + startup drain + hourly raw-MIME prune), `OnModuleInit` | Durable, fail-closed inbound via `InboundDelivery`: accepts each IMAP/PIPE transport copy under a unique key, advances cursor only after durable acceptance, then drains under a heartbeat lease. Real RFC Message-IDs create/read an atomic `InboundMessageClaim` (same normalized semantics → SKIPPED; semantic conflict → audited quarantine); headerless mail is idempotent only by transport identity. Owner routing is deterministic (`EmailQueue.routingPriority`, then queue id) and persisted before ticket work. Also includes truncated-fetch fast quarantine, sender-authorized threading, bounded MIME parsing, bootstrap/UIDVALIDITY protections, liveness and raw-MIME retention. |
...
| `MailProcessor`      | BullMQ queue `mail`, per-job                                                                   | Claims durable `OutboundEmail` rows using a DB lease/fence, streams immutable attachment snapshots to Nodemailer and marks `SENT` only after SMTP accepts. Generic legacy template jobs remain supported for compatibility.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
