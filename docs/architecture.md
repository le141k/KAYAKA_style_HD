...
| `MailModule`           | `src/modules/mail/`           | Inbound IMAP/PIPE durable ledger plus SMTP. A public staff reply creates `OutboundEmail` atomically with its post; BullMQ carries only its id and DB recovery scans authoritative rows after Redis/worker restart. Legacy system templates remain compatible but are not durable outbox commands. |
