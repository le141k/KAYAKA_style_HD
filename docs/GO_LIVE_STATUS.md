...
| Inbound mail       | IMAP and webhook share a durable, bounded acceptance/drain path; epoch/generation fencing prevents stale cursor writes; poison messages are retried then quarantined; real Message-ID claims use semantic content while headerless mail stays transport-identity based. Large raw MIME is retained privately in uploads with bounded cleanup. |
